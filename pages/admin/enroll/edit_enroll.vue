    ...
    <div>
        <v-container>
            ...
            <v-form>
                ...
                <v-row class="mt-9">
                    <v-btn color="success" @click="enroll()" block>enroll</v-btn>
                </v-row>
        </v-container>
    </div>
</template>
    ...
    methods: {
        ...
        enroll(){
            for (let i = 0; i < 35; i++){
                ...
                if (this.time_table[i].enroll_subject_id === ""){
                    continue;
                }
                axios.post('http://localhost/service_ip3/admin/enroll/auto_enroll.php',
                this.time_table[i]
                )
                .then((resp) => {
                        console.log(resp);
